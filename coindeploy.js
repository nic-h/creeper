...
if (!RPC_URL || !PRIVATE_KEY || !COIN_ADDRESS || !PINATA_API_KEY || !PINATA_API_SECRET) {
  console.error('Error: Missing environment variables. Check RPC_URL, PRIVATE_KEY, COIN_ADDRESS, PINATA_API_KEY, PINATA_API_SECRET.')
  process.exit(1)
}
...
// ─── Main: Pin image, metadata, and update on-chain ────────────────────────────
async function main() {
  try {
    // 1) Pin the latest snapshot image
    const imagePath = path.resolve('snapshots', 'creeper.png')
    console.log('→ Pinning image:', imagePath)
    const pinFileResponse = await pinata.pinFileToIPFS(
      fs.createReadStream(imagePath)
    )
    const imageCID = pinFileResponse.IpfsHash
    ...
    const pinJsonResponse = await pinata.pinJSONToIPFS(metadata)
    const metadataCID = pinJsonResponse.IpfsHash
